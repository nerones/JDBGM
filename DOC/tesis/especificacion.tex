...
Cuando se habla de ocultar el acceso a el motor se esta haciendo referencia a que la API proveída por este proyecto debe en primer termino ocultar el uso de JDBC en el sentido de que no se hace uso explicito de los métodos que este ofrece, en segundo lugar se refiere a que los métodos proveídos por la API aquí desarrollada no deben ser ``motor dependiente'' para evitar que el código también lo sea. Para manejar el acceso al motor de base de datos \jj debe proveer las siguientes funcionalidades mínimas que a su vez puede que se desglosen en mas de una función (o método):
\begin{enumerate}

\item Crear una conexión con el motor: es decir se debe encargar de obtener una conexión al motor y ponerla a disponibilidad de los métodos definidos.
...
\item Poder enviar transacciones: una transacción consiste en un conjunto de sentencias que deben ejecutarse en conjunto, una vez finalizadas se guardan los cambios (commit) o se rechazan (rollback) si sucediere algún error.

\item Manejar los errores que se puedan producir: se pueden dar errores en la sintaxis de las sentencias SQL como así también errores en la lógica de las sentencias que se formaron, como por ejemplo equivocarse en el nombre de una tabla o referirse a una columna inexistente.
...
\section{Eliminando dialectos} \label{seccion:especificacion:dialectos}
En el capitulo anterior se comento que parte del desafío de nuestro proyecto era lidiar con los diferentes  dialectos utilizados por cada motor por lo que a continuación analizaremos como es que pretendemos sortear esta dificultad.

JDBC nos provee de la clase \verb=Statement=\footnote{Posteriormente explicare como es que se trabaja con JDBC por ahora interesa solo lo expuesto} la cual posee los métodos \verb=executeUpdate= y \verb=executeQuery= los cuales permiten enviar sentencias SQL al motor de base de datos. El envío de estas sentencia se hace mediante cadenas de texto por lo que se nos presenta el problema de que estas cadenas ``estáticas'' dependen, según cuan complejas sean las sentencias, de el motor en particular que se este usando es por ello que en este proyecto se opto por generar las sentencias sobre demanda, es decir se pretende que las sentencias se almacenen desglosadas de modo que su sintaxis quede expuesta y pueda luego ser volcada en una cadena de texto cuando sean requeridas por el programador. El almacenar de este modo las sentencias no asegura tampoco la independencia de los dialectos usados pero nos permite manejar el modo en que las sentencias son convertidas en cadenas de textos, entonces si se define un nuevo dialecto que sea compatible con los dialectos usados por los otros motores y ajustando la estructura de datos que se va a utilizar para las sentencias a este dialecto se puede luego mediante las reglas adecuadas "traducir" la sentencia a su dialecto correspondiente. 

Definir un nuevo dialecto puede parecer algo engorroso, pero en realidad este nuevo dialecto estará basado en los dialectos usados por los motores a los que se le esta dando soporte en este proyecto y además solo se trabajara con un subconjunto de sentencias definidos en estos dialectos pues no es objetivo del proyecto brindar un total soporte sobre SQL si no que se cubrirán las siguientes sentencias:
\begin{itemize}
...
Una vez definido el subconjunto de sentencias con las que se trabajaran es necesario definir el dialecto genérico sobre el cual se apoyara este proyecto por lo que para cada una de las sentencias se definirá un comportamiento común, pero para que esto sea posible deberemos restringir las dialectos soportados al que menos posibilidades presente pues las diferencias entre los dialectos usados por los motores se da generalmente por que se agregan funcionalidades a las definidas en el estándar  SQL y raramente se da el caso en que la diferencia se de por la falta de implementación de lo definido en el estándar. Como parte del análisis de las diferencias entre los dialectos y mas allá de la diferencia entre las sintaxis usadas para las sentencias es importante analizar también los tipos de datos que se usan en cada \dd. También para ellos debemos encontrar un factor común, por lo que a continuación dedicamos un capitulo entero a analizar los tipos de datos usados por cada motor.
...
\subsection{Tipos de datos y sus diferencias}\label{subsection:datatypes}
...
\subsubsection{Especificación de CREATE TABLE} \label{especificacion:dialectos:create}
...
\subsubsection{Especificación de UPDATE} \label{especificacion:dialectos:update}
...
\subsubsection{Especificación de INSERT} \label{especificacion:dialectos:insert}
...
\subsubsection{Especificación de ALTER TABLE} \label{especificacion:dialectos:altertable}
...
\subsubsection{Especificación de DELETE} \label{especificacion:dialectos:delete}
...
\subsubsection{Especificación de SELECT} \label{especificacion:dialectos:select}
